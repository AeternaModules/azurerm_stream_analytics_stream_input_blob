...
output "stream_analytics_stream_input_blobs_serialization" {
  ...
  value       = { for k, v in azurerm_stream_analytics_stream_input_blob.stream_analytics_stream_input_blobs : k => one(v.serialization) if v.serialization != null && length(v.serialization) > 0 }
}
